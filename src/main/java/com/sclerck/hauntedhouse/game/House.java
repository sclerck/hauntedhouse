...
public class House {
    ...
    public void setup(HauntedHouseService service) {
        ...
        // Floor 1 rooms
        Room open = RoomBuilder.build("white room", "This is the white room.", floor1, false, Colour.WHITE, new HashMap<>(Map.of("key", new Key(Colour.RED))));
        Room red = RoomBuilder.build("red room", "This is the red room.", floor1, true, Colour.RED, new HashMap<>(Map.of("key", new Key(Colour.GREEN))));
        ...
        SecretRoom basement = SecretRoomBuilder.build("basement", "This is the basement.", floor1, true, Colour.BLACK, new HashMap<>(Map.of("key", new Key(Colour.YELLOW))), inventory -> inventory.getKey("basement") != null);
